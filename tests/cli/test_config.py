...
def test_config_get_value(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = CliRunner().invoke(main, ["config", "get", "PYGENT_TEST_VALUE"])
    assert result.exit_code == 0
...
def test_config_get_secret_is_masked(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = CliRunner().invoke(main, ["config", "get", "PYGENT_TEST_API_KEY"])
    assert result.exit_code == 0
...
def test_config_list_uses_requested_locale(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("PYGENT_MODEL", "test-model")
    result = CliRunner().invoke(main, ["--locale", "ja", "config", "list"])
    assert result.exit_code == 0
    assert "モデル" in result.output
...
def test_load_config_missing(tmp_path: Path) -> None:
    ...
    assert config.provider.model == "qwen2.5-coder:3b"
...
def test_config_init_command(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    config_file = tmp_path / "pygent" / "config.toml"
    monkeypatch.setattr("pygent.cli.commands.config.config_path", lambda: config_file)
    monkeypatch.setattr(
